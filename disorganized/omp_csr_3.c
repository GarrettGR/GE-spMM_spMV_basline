...
void sparse_matrix_mult(sparse_matrix A, sparse_matrix B, sparse_matrix *C) {
  ...
  int num_c_elems = C->row_ptr[m];
  C->col_ind = (int*) malloc(num_c_elems * sizeof(int));
  C->val = (double*) calloc(num_c_elems, sizeof(double));
  ...
  double total_mem = (a_nonzeros + b_nonzeros + c_nonzeros) * (sizeof(double) + sizeof(int));
...
sparse_matrix csr_from_dense(double *mat, int m, int n) {
  int nnz = 0;
  for (int i = 0; i < m*n; i++) {
    if (mat[i] != 0) nnz++;
  }
...
int main() {
  ...
  sparse_matrix A = csr_from_dense(mat_a, m, n);
  sparse_matrix B = csr_from_dense(mat_b, m, n);
  sparse_matrix C;
